namespace ATMStarbank;

class Menudebito
{
    static void Main(string[] args)
    {
        string Opciones;

        Console.WriteLine("Trasacciones con Tarjeta de Debito");
        Console.WriteLine("Elija la operacion que desea realizar");
        Console.WriteLine("1.Disponibilidad de efectivo"); //Eliminalo
        Console.WriteLine("2.Transacciones"); //eliminalo
        Console.WriteLine("3.Retiro Efectivo");
        Console.WriteLine("4.Depositar en Efectivo");
        Console.WriteLine("5.Consultar Saldo");
        Console,WriteLine("6.Registrar un nuevo PIN"); 
        Console.WriteLine("7.Pago de Servicios");
        Console.WriteLine("8.Regresar");
            Opciones = Console.ReadLine();
            
                switch (Opciones)
                {
                    case "1":
                    
                        
                        break;
                    
                    case "2":
                    
                        
                        break;
                    
                    case "3":
                    
                        
                        break;
                    
                    case "4":
                    
                        
                        break;
                    
                    case "5": 

                        break;

                    case "6": 

                        break;
                        
                    case "7": 

                        break;

                }
                if (Opciones =="8")
                {
                Console.WriteLine("");//mike aqui pondras para que regrese al apartado anterior 
                } 
     }
}
